        ...
        if (is_blank_column(image, x) || x == image->w) {
            if (x == save_index) {
                ...
                letters_list = push_back_list(letters_list,
                                              (void *) cut_image(image, save_index, 0, x - save_index, image->h),
                                              LetterType);
                save_index = x + 1;
            }
        }
...
List get_words_and_letters(SDL_Surface *image) {
   ...
   // space_limite = (int) (space_limite *0,90);
    ...
    // SDL_SaveBMP(get_all_text(image,10),"Cocospace.jpg");
    ...
    for (int x = 0; x <= image->w; ++x) {
        ...
        if (blank_count >= space_limite || x== image->w) {
            words_list = push_back_list(words_list,
                                        (void *) get_letters(cut_image(image, save_index, 0, x - save_index, image->h)),
                                        WordType);
            save_index = x + 1;
            blank_count = 0;
        }
